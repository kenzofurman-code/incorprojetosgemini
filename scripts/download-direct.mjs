...
async function downloadDirect() {
  console.log('🚀 Baixando fotos diretamente...')
  ...
  try {
    ...
    // Clicar no botão da UCE (comentário de 29/05/2026)
    const uceCard = page.locator('text=Secretaria Municipal de Urbanismo - Departamento de Controle de Edificações').first()
    await uceCard.scrollIntoViewIfNeeded()
    await page.waitForTimeout(1000)

    // Clicar no botão Visualizar próximo ao card da UCE
    const btn = page.locator('sy-button:has-text("Visualizar")').nth(2) // 3º botão
